...
import { trackCouponApplied, trackCouponFailed } from "@/services/analytics";
...
const COUPONS: Coupon[] = [
  { code: "BOGIE100", discount: 100, type: "flat",    descriptionKey: "cab.coupons.catalog.bogie100", minFare: 150 },
  { code: "RIDE10",   discount: 10,  type: "percent", descriptionKey: "cab.coupons.catalog.ride10",    minFare: 200 },
  { code: "NEWUSER",  discount: 150, type: "flat",    descriptionKey: "cab.coupons.catalog.newuser",   minFare: 250 },
];

function computeDiscount(coupon: Coupon, total: number): number {
  if (coupon.type === "flat")    return coupon.discount;
  if (coupon.type === "percent") return Math.round(total * coupon.discount / 100);
  return 0;
}

export default function CabCouponsScreen() {
  ...
  const applyAndReturn = async (coupon: Coupon) => {
    ...
    trackCouponApplied({ couponCode: coupon.code, discountAmount: discount, service: "cab" });
  ...
  const applyManual = async () => {
    ...
    if (!found) {
      trackCouponFailed({ couponCode: code, reason: "invalid_code" });
      setManualError(t("booking.coupons.invalidCode")); return;
    }
    if (total < found.minFare) {
      trackCouponFailed({ couponCode: code, reason: `min_fare_${found.minFare}` });
      setManualError(t("booking.coupons.minFareRequired", { amount: found.minFare }));
      return;
    }
    setManualError("");
            ...
            <View key={coupon.code} style={[s.couponCard, !eligible && s.couponCardDisabled]}>
              ...
              <View style={s.couponTop}>
                <View style={s.couponLeft}>
                  ...
                  {!eligible && (
                    <Text style={s.minFare}>{t("booking.coupons.minFareRequired", { amount: coupon.minFare })}</Text>
                  )}
                </View>
                <View style={s.couponRight}>
                  ...
                  {eligible && <Text style={s.savesText}>{t("booking.coupons.savesAmount", { amount: discount })}</Text>}
                </View>
              </View>
...
const s = StyleSheet.create({
  ...
  manualInput: {
    ...
    color: COLORS.textPrimary, fontSize: 15, letterSpacing: 1,
  },
  manualBtn: {
  ...
  notchLeft: {
    ...
    borderWidth: 1, borderColor: "#E0E0E0", zIndex: 2,
  },
  notchRight: {
  ...
  couponLeft:  { flex: 1 },
  couponCode:  { color: COLORS.textPrimary, fontSize: 18, fontWeight: "900", letterSpacing: 1 },
  ...
  applyBtn:     { alignItems: "center", paddingVertical: 12 },
